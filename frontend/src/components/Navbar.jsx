...
const DropdownMenu = ({ title, path, items }) => {
  ...
  return (
    <div className="relative group/main">
      {isExt ? (
        <a href={path} target="_blank" rel="noopener noreferrer" className={triggerClass}>
          {title}
      ...
      <div className="absolute top-full left-0 bg-[#0f3358] shadow-2xl min-w-52 xl:min-w-60 z-[999] border-2 border-[#d4af37] rounded-b-xl hidden group-hover/main:block">
        {items.map((item, idx) => {
          const isItemExt = isExternalUrl(item.path) || item.target === '_blank';
          return item.children ? (
            <SubMenu key={item.label || idx} label={item.label} path={item.path} children={item.children} />
          ) : isItemExt ? (
            <a
              key={item.path || idx}
              href={item.path || "#"}
    ...
    return (
      <div>
        <div className="flex items-center justify-between">
          {item.path ? (
            isExt ? (
              <a
                ...
                rel="noopener noreferrer"
                onClick={onNavigate}
                className={`flex-1 min-w-0 py-3 text-xs sm:text-sm text-left transition-colors cursor-pointer ${style.bg} ${style.text} ${style.weight} ${style.hover}`}
            ...
            <span
              onClick={() => setOpen((o) => !o)}
              className={`flex-1 min-w-0 py-3 text-xs sm:text-sm text-left transition-colors cursor-pointer ${style.bg} ${style.text} ${style.weight}`}
              style={{ paddingLeft: `${indentPad}px` }}
            >
              <span className={depth === 0 ? "font-cinzel tracking-wide" : ""}>{item.label}</span>
            </span>
          ...
          <button
            onClick={() => setOpen((o) => !o)}
            className={`py-3 px-4 shrink-0 text-[#d4af37] ${style.bg} ${style.hover} cursor-pointer`}
            aria-expanded={open}
...
// ============================================
// MOBILE MENU PANEL  (shows below `lg`)
// ============================================
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b-2 border-[#d4af37] sticky top-0 bg-[#0f3358] z-10 shadow-md">
          ...
          <button
            ...
            className="shrink-0 text-slate-300 hover:text-white text-xl leading-none w-8 h-8 flex items-center justify-center rounded-lg border border-slate-700 bg-white/5"
          >
            ✕
          </button>
        </div>
...
const Navbar = () => {
  ...
  // Breakpoint matches the `lg` switch used for the nav/hamburger.
  ...
  // Close the drawer automatically once we cross into desktop width.
  useEffect(() => {
      ...
      >
        <div className="overflow-hidden">
          <header className="w-full bg-[#0f3358]">
            ...
            <div className="relative bg-white px-3 sm:px-6 xl:px-8 py-2.5 sm:py-3 flex items-center justify-between gap-2 sm:gap-3 border-b border-[#d4af37]/40 shadow-sm">
              {/* BHU logo (always left) */}
              ...
              {/* Desktop (lg and up) right side: Title + MMV logo */}
              <div className="hidden lg:flex items-center gap-3 min-w-0">
                <div className="min-w-0 text-right">
                  <h1 className="text-[#353375] text-lg xl:text-2xl font-bold font-cinzel leading-tight tracking-wide">
                    Mahila Mahavidyalaya
                  </h1>
                  <p className="text-[11px] xl:text-xs text-[#353375] font-semibold tracking-wider uppercase">
                    Varanasi
                  </p>
                </div>
                <a
                  href="https://www.bhu.ac.in/site/UnitHomeTemplate/1_184_1231_Mahila-Maha-Vidyalaya-Home"
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label="Visit MMV Website"
